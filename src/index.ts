...
const PREREQ_NAME = "gh-copilot";
const CLI_INSTALL_COMMAND = ["gh", "extension", "install", "github/gh-copilot"];
...
class ProviderImpl implements AIAgentProvider {
  ...
  async healthCheck(): Promise<{ ok: boolean; message?: string }> {
    ...
    return version
      ? { ok: true, message: `${DISPLAY} ${version}` }
      : { ok: false, message: `${DISPLAY} GitHub CLI extension not available` };
  }
          ...
          : [
              {
                ...
                requiresSudo: false,
                description: `${DISPLAY} GitHub CLI extension for CLI mode`,
              },
            ],
      };
    })
...
type CopilotVibePlugin = VibePlugin & {
