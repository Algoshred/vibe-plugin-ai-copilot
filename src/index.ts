 ...
 * vibe-plugin-copilot
 *
 * GitHub Copilot AI agent provider for VibeControls Agent.
 * Implements the AIAgentProvider interface to manage Copilot sessions.
 * Uses the `github-copilot-cli` with `what-the-shell` subcommand.
 */
...
const CLI_COMMAND = "github-copilot-cli";
...
class CopilotProvider implements AIAgentProvider {
  ...
  private buildCliArgs(_config: AISessionConfig, prompt: string): string[] {
    const args: string[] = ["what-the-shell", prompt];
    return args;
  }
...
export const vibePlugin: VibePlugin = {
  name: "copilot",
  version: "1.0.0",
  description: "GitHub Copilot AI agent provider for VibeControls",
  ...
  onServerStop() {
    for (const [id] of (provider as CopilotProvider)["sessions"]) {
